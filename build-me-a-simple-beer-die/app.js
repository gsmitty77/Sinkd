...
const achievementDefinitions = [
  { key: "sinks", label: "Sink", shortLabel: "SNK", thresholds: [5, 15, 30, 50], icon: "S" },
  { key: "tinks", label: "Tink", shortLabel: "TNK", thresholds: [10, 30, 60, 100], icon: "T" },
  { key: "fgOffense", label: "FG Off", shortLabel: "OFF", thresholds: [15, 40, 80, 150], icon: "O" },
  { key: "fgDefense", label: "FG Def", shortLabel: "DEF", thresholds: [10, 25, 50, 90], icon: "D" },
  { key: "fifas", label: "FIFA", shortLabel: "FIF", thresholds: [25, 75, 150, 300], icon: "F" },
  { key: "selfSinks", label: "Self Sink", shortLabel: "SLF", thresholds: [3, 8, 15, 25], icon: "!" },
  { key: "tableHits", label: "Table Hit", shortLabel: "HIT", thresholds: [25, 100, 200, 500], icon: "H" },
];
...
function bindEvents() {
  ...
  document.addEventListener("click", (event) => {
    ...
    form.querySelectorAll("[data-self-sink]").forEach((selfSinkButton) => {
      selfSinkButton.classList.toggle("active", selfSinkButton === button);
    });
...
function rememberPlayers(players) {
  state.players = mergePlayerNames([...collectAllPlayerNames(), ...players]);
  localStorage.setItem(playerRosterKey, JSON.stringify(state.players));
}
...
function createTournament(name, teams) {
  ...
  const tournament = {
    id: crypto.randomUUID(),
    name: cleanText(name) || "Beer Die Tournament",
    createdAt: new Date().toISOString(),
    teams,
    rounds,
  };
...
function friendRow(request) {
  ...
  const stats = computePlayerStats()[profileKey(friend.name)]?.overall || emptyBucket();
  ...
  return `
    <section class="achievement-section">
      ...
      <div class="achievement-grid">
        ${achievementDefinitions.map((definition) => achievementCard(definition, stats)).join("")}
      </div>
    </section>
  `;
}
...
function achievementCard(definition, stats) {
  ...
  return `
    <article class="achievement-card achievement-${progress.tierClass}">
      ...
      <b>${value}</b>
      <small>${escapeHtml(progress.progressText)}</small>
    </article>
  `;
}

function achievementBadge(definition, progress) {
  const badgeSrc = achievementBadgeSrc(progress.tierClass);
  return `
    <span class="achievement-badge achievement-badge-${progress.tierClass}" aria-hidden="true">
      ${badgeSrc ? `<img src="${badgeSrc}" alt="" />` : ""}
      <span class="achievement-badge-code">${escapeHtml(definition.shortLabel)}</span>
    </span>
  `;
}
